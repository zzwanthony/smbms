...
//			validateTip(providerId.next(),{"color":"#F7FF00"},imgNo+" 获取供应商列表error",false);
//		}
//	});
	...
	/*billCode.bind("blur",function(){
		if(billCode.val() != null && billCode.val() != ""){
			proCodeAndName(billCode)
			validateTip(billCode.next(),{"color":"green"},imgYes,true);
		}else if( billCode.val() == ""){
			validateTip(billCode.next(),{"color":"#666666"},"* 请输入订单编码编码，",false);
		}
		else{
			validateTip(billCode.next(),{"color":"#F7FF00"},imgNo+" 编码不能为空，请重新输入",false);
		}
	}).bind("focus",function(){
		//显示友情提示
	...
	productName.on("focus",function(){
		...
		if(productName.val() != null && productName.val() != ""){
			...
			validateTip(productName.next(),{"color":"#F7FF00"},imgNo+" 商品名称不能为空，请重新输入",false);
		}
	...
	productUnit.on("focus",function(){
		...
		if(productUnit.val() != null && productUnit.val() != ""){
			...
			validateTip(productUnit.next(),{"color":"#F7FF00"},imgNo+" 单位不能为空，请重新输入",false);
		}
	...
	providerId.on("focus",function(){
		...
		if(providerId.val() != null && providerId.val() != "" && providerId.val() != 0){
			...
			validateTip(providerId.next(),{"color":"#F7FF00"},imgNo+" 供应商不能为空，请选择",false);
		}
	...
	productCount.on("focus",function(){
		...
		if(productCount.val()==null || productCount.val()==""){
			validateTip(productCount.next(),{"color":"#F7FF00"},imgNo+" 商品数量不能为空，请重新输入",false);
		}else{
			if(this.value>0){
				validateTip(productCount.next(),{"color":"green"},imgYes,true);
			}else{
				validateTip(productCount.next(),{"color":"#F7FF00"},imgNo+" 商品数量要大于0，请重新输入",false);
			}
		}
	});
	...
	totalPrice.on("focus",function(){
		...
		if(totalPrice.val()==null || totalPrice.val()==""){
			validateTip(totalPrice.next(),{"color":"#F7FF00"},imgNo+" 总金额不能为空，请重新输入",false);
		}else{
			if(this.value>0){
				validateTip(totalPrice.next(),{"color":"green"},imgYes,true);
			}else{
				validateTip(totalPrice.next(),{"color":"#F7FF00"},imgNo+" 总金额要大于0，请重新输入",false);
			}
		}
	});
