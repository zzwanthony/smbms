...
//			validateTip(userRole.next(),{"color":"#F7FF00"},imgNo+" 获取用户角色列表error",false);
//		}
//	});
	...
	userCode.bind("blur",function(){
		...
		if(userCode.val()==""){
			validateTip(userCode.next(),{"color":"#F7FF00"},imgNo+ " 用户账号不能为空",false);
		}else{
			$.ajax({
				type:"GET",//请求类型
				url:path+"/userManager/userNameTest",//请求的url
				data:{method:"ucexist",userCode:userCode.val()},//请求参数
				dataType:"json",//ajax接口（请求url）返回的数据类型
				success:function(data){//data：返回数据（json对象）
					if(data == "exist"){//账号已存在，错误提示
						validateTip(userCode.next(),{"color":"#F7FF00"},imgNo+ " 该用户账号已存在",false);
...
//					validateTip(userCode.next(),{"color":"#666666"},"* 用户编码是您登录系统的账号",false);
				...
				error:function(data){//当访问时候，404，500 等非200的错误状态码
					validateTip(userCode.next(),{"color":"#F7FF00"},imgNo+" 您访问的页面不存在",false);
				}
			});
		}
	}).bind("focus",function(){
	...
	userName.bind("focus",function(){
		...
		if(userName.val() != null && userName.val().length > 1
			...
			validateTip(userName.next(),{"color":"#F7FF00"},imgNo+" 用户名输入的不符合规范，请重新输入",false);
		}
	});
	...
	userPassword.bind("focus",function(){
		...
		if(userPassword.val() != null && userPassword.val().length > 6
			...
			validateTip(userPassword.next(),{"color":"#F7FF00"},imgNo + " 密码输入不符合规范，请重新输入",false);
		}
	});
	...
	ruserPassword.bind("focus",function(){
		...
		if(ruserPassword.val() != null && ruserPassword.val().length > 6
			...
			validateTip(ruserPassword.next(),{"color":"#F7FF00"},imgNo + " 两次密码输入不一致，请重新输入",false);
		}
	});
	...
	birthday.on("focus",function(){
		...
		if(birthday.val() != null && birthday.val() != ""){
			...
			validateTip(birthday.next(),{"color":"#F7FF00"},imgNo + " 选择的日期不正确,请重新输入",false);
		}
	});
	...
	phone.bind("focus",function(){
		...
		if(phone.val().match(patrn)){
			...
			validateTip(phone.next(),{"color":"#F7FF00"},imgNo + " 您输入的手机号格式不正确",false);
		}
	});
	...
	userRole.bind("focus",function(){
		...
		if(userRole.val() != null && userRole.val() > 0){
			...
			validateTip(userRole.next(),{"color":"#F7FF00"},imgNo + " 请重新选择用户角色",false);
		}
	});
